...
//    for quntity control in cart ENDS
        ...
        if($r->has('order'))
        {
            if(Session::has('user'))
            {
                ...
                foreach ($totalCart as $c) {
                    ...
                    $res = Product::find($a[0]);
                    $product[] = $res;
                    $bought_price = $a[1] * $res->per_price;
                    $bought_cost+= $bought_price;
                    Session::put('bought_price', $bought_cost);

                    $stock = $res->total_stock - $a[1];
                ...
                if ($r->payment == 'Paid')
                {
                    ...
                    $sales->bought_price = session('bought_price');
                    $sales->payment_status = $r->payment;
                    $sales->save();

                    $insertedId = $sales->id;
                    Session::put('last_id', $insertedId);
    ...
    public function history(Request $r)
    {
        ...
        $res2 = Service::where('user_id', session('user')->id)->get();
        if(!$res1)
        {
        ...
        if (!$res2) {
            return view('user.serviceHistory')->with('all', [])
                ->with('products', [])
                ->with('Service', []);
        }
        ...
        foreach ($res2 as $r) {
            ...
            foreach ($totalService as $c) {
                $book[] = array_prepend(explode(':', $c), $r->id);
                // $a = explode(':', $c);
                // $res = Service::find($a[0]);
                // $product[] = $res;
            }
        }
        $res = Product::all();
    ...
    public function serviceIndex(Request $r)
    {
        $cat = Category::all();
        return view('services.service')
        ->with('cat', $cat);
    }
    public function confirmService(Request $r)
    {
        if ($r->has('book')) {
            if (Session::has('user')) {

                $services = new service();
                $services->user_id = session('user')->id;
                $services->booking_date = $r->date;
                $services->booking_time = $r->time;
                $services->service_cat = $r->service;
                $services->service_details = $r->details;
                $services->booking_status = 'Placed';
